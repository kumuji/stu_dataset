...
def process_frame(
    ...
    if np.sum(pred_mask) > 0:
        clusters = (
            DBSCAN(eps=1.0, min_samples=1, n_jobs=-1, leaf_size=100)
            .fit(points[pred_mask][~distance_mask])
            .labels_
        )
        instances[pred_mask][~distance_mask] = clusters + 1  # Cluster IDs start at 1
        semantics[pred_mask][~distance_mask] = 1  # single class, that we are evaluating
